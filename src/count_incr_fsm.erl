%% -------------------------------------------------------------------
%%
%% count_incr_fsm: Co-ordinating, per request, state machine for increments
%%
...
%%% API
%%%===================================================================
start_link(ReqID, From, Key, Amount, Timeout) ->
    gen_fsm:start_link(?MODULE, [ReqID, From, Key, Amount, [{timeout, Timeout}]], []).
...
init([ReqID, From, Key, Amount, Opts]) ->
...
prepare(timeout, SD0=#state{key=Key, from=From, amount=Amount,
    ...
    case {Primaries, Preflist2, LocalPL =:= []} of
        ...
        {_, _, true} ->
            ...
            case count_incr_fsm_sup:start_incr_fsm(CoordNode, [ReqId, From, Key,
                                                                     Amount, Timeout]) of
                {ok, _Pid} ->
                    {stop, normal, SD0};
                {error, Reason} ->
...
execute(timeout, SD=#state{coord_pl_entry=CoordPLEntry,
    ...
    case count_vnode:increment(CoordPLEntry, Key, Amount, Timeout) of
        {ok, Downstream} ->
            %% ask remote nodes to merge
            PrefList2 = [{Index, Node} || {{Index, Node}=Entry, _Type} <- PrefList,
                                          Entry =/= CoordPLEntry],
            count_vnode:merge(PrefList2, Key, Downstream, ReqId),
